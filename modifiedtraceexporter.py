...
def disturb_gps(x, y, deviation,lst1=[],lst2=[]):
        latituderadians  = 0.01745329 * y
        longituderadians = 0.01745329 * x
        a = 6378137

        x = a  * math.cos (latituderadians) * math.cos (longituderadians)
        y = a  * math.cos (latituderadians) * math.sin (longituderadians)
        z = a  * math.sin (latituderadians)
...
"""
def offset(latitude,longitude,deviation,listoflat,listoflong):
    latituderadians = 0.01745329 * longitude
    longituderadians = 0.01745329 * latitude
    a = 6378137


    latitude = a * math.cos(latituderadians) * math.cos(longituderadians)
    longitude = a * math.cos(latituderadians) * math.sin(longituderadians)
    z = a * math.sin(latituderadians)

    latitude += random.gauss(0, deviation)
    longitude += random.gauss(0, deviation)
    longitude = -longitude

    latitude=latitude-listoflat[0]
    longitude=longitude-listoflong[0]
    comb=zip(listoflat,listoflong)
    l=len(comb)

    with open('cellTowersForRoute.csv', 'rb') as fin, open('cellTowersForRouteCorrectPosition.csv', 'w') as fout:
        reader = csv.DictReader(fin, delimiter=',', quotechar='"')
        headers = reader.fieldnames
        # next(reader, None)
        wr = csv.DictWriter(fout,fieldnames=headers,delimiter=',', quotechar='"')
        wr.writeheader()
        for row in reader:
            a1 = row['LATITUDE']
            b1 = row['LONGITUDE']
            gpslat = float(a1)
            gpslong = float(b1)
            latitudeforcell = 0.01745329 * gpslat
            longitudeforcell = 0.01745329 * gpslong
            cart_latitude_cell = a * math.cos(latitudeforcell) * math.cos(longitudeforcell)
            cart_longitude_cell = a * math.cos(latitudeforcell) * math.sin(longitudeforcell)
            "z = a * math.sin(latitudeforcell)"
            cart_longitude_cell = -cart_longitude_cell
            cart_latitude_cell = cart_latitude_cell - listoflat[0]
            cart_longitude_cell = cart_longitude_cell - listoflong[0]
  	    #cart_latitude_cell = -cart_latitude_cell
            cart_longitude_cell = -cart_longitude_cell
            row['LATITUDE'] = cart_latitude_cell
            row['LONGITUDE'] = cart_longitude_cell
            wr.writerow(row)
...
def makeEntries(movables, chosen, options):
    ...
    for v in movables:
        ...
        if chosen[v.id]:
            "x, y, lst = disturb_gps(float(v.x), float(v.y), options.blur)"
            a, b=disturb_gps(float(v.x), float(v.y), options.blur)
            x, y = offset(float(v.x),float(v.y),options.blur,a ,b )
